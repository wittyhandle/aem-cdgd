...
/**
 * This is a customization for DTNA:
 *
 * Allow authors to perform Active Later operations at the time of workflow
 * start.
 *
...
CQ.wcm.Sidekick = CQ.Ext.extend(CQ.Dialog, {
    ...
    /**
     * @cfg {String} sitecatalystText
     * The text for the SiteCatalyst button (defaults to "SiteCatalyst").
     */
    sitecatalystText: null,
     ...
     * {@link CQ.wcm.Sidekick#DEFAULT_ACTIONS}).
     * <p>Example:
     * <pre><code>
[
...
//                    console.log(e);
            ...
            "handler": function() {
                ...
                dialog.success = function(form,slingSubmit) {
                    if( slingSubmit && slingSubmit.result &&  slingSubmit.result["Path"]) {
                        CQ.Util.reload(CQ.WCM.getContentWindow(), CQ.HTTP.externalize(slingSubmit.result["Path"] + CQ.HTTP.EXTENSION_HTML));
                    } else {
     ...
     * dialog.
     * @private
     * @param force if true, no reference check is performed
     */
     ...
     */
    getRolloutConfig: function() {
        var currentPage = CQ.WCM.getPage(this.path);
        return {
            "disabled": !currentPage.isLiveCopySource(),
            "text": this.rolloutText,
            "handler": function() {
                var sidekick = CQ.wcm.Sidekick.findSidekick(this);
                var path = this.getPath();

                this.rolloutWizard = new CQ.wcm.msm.RolloutWizard({"path": path});
    ...
    loadWorkflowTab: function() {
        ...
        if (!isInWorkflow) {
            ...
            var wData = pageInfo.workflows.wcm.models;
            ...
            var twData = pageInfo.workflows.translation.models;
            ...
            workflowConsolePanel.items = [
                ...
                },
                {
                    ...
                    "items": [
                        {
                            "xtype":"datetime",
                            "fieldLabel": "Active Later",
                            "id":"dtna.activatelater",
                            "dateWidth": 85,
                            "timeWidth": 85,
                            "value": "now"
                        }
                ...
                if (languages){
                    for (var i = 0; i<languages.length; i++){
                        ...
                        if(row['path'] != this.path){
                            ...
                            if(row['exists']){
                                cls += "color:black;";
                                img = "<a href='" + CQ.shared.HTTP.getXhrHookedURL(CQ.HTTP.externalize(row['path']) + ".html") + "' target='_new' style=\'"+cls+"\' title='"+row['language']+"'><img style=\'vertical-align:text-bottom;width:20px;margin-left:5px;\' src=\'" + CQ.shared.HTTP.getXhrHookedURL(flagRoot + row['country'] + ".gif") + "\'></a>";
                            }
                            else{
                                cls += "color:#888888;";
                                img = "<a href='" + CQ.shared.HTTP.getXhrHookedURL(CQ.HTTP.externalize(row['path']) + ".html") + "' target='_new' style=\'"+cls+"\' title='"+row['language']+"'><img style=\'vertical-align:text-bottom;opacity:0.4;filter:alpha(opacity=40);width:20px;margin-left:5px\' src=\'" + CQ.shared.HTTP.getXhrHookedURL(flagRoot + row['country'] + ".gif") + "\'></a>";
                            }
                            var lbl = "<span style=\'"+cls+"\'><a href='" + CQ.shared.HTTP.getXhrHookedURL(CQ.HTTP.externalize(row['path']) + ".html") + "' target='_new' title='"+row['language']+"' style=\'"+cls+"\'>"+row['language']+"</a></span>";
                            langOptions.push({
                                "text": img+" "+lbl,
                                "value": row['path']
                            });
...
//        this.checkPosition();
    ...
    constructor: function(config) {
        ...
        CQ.Util.applyDefaults(config, {
            ...
            "sitecatalystText": CQ.I18n.getMessage("SiteCatalyst"),
 ...
 * <ul>
 * <li>{@link CQ.wcm.Sidekick#PROPS} Properties button</li>
 * <li>{@link CQ.wcm.Sidekick#DELETE} Delete button</li>
 * <li>{@link CQ.wcm.Sidekick#PUBLISH} Publish button</li>
 * <li>{@link CQ.wcm.Sidekick#LOCK} Lock button</li>
 * <li>{@link CQ.wcm.Sidekick#REFERENCES} References button</li>
 * <li>{@link CQ.wcm.Sidekick#ROLLOUT} Rollout button</li>
 * <li>{@link CQ.wcm.Sidekick#AUDIT} Audit Log button</li>
 * <li>{@link CQ.wcm.Sidekick#PERMS} Permissions button</li>
 * <ul>
 * @static
 * @final
 * @type String[]
...
CQ.wcm.Sidekick.DragZoneBase = {
     ...
     * @params {CQ.wcm.EditBase.DropTarget} dropTarget The calling drop target.
